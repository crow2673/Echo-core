...
def get_echo_self_knowledge() -> dict:
    """Pull real data about Echo's history, memory, income, and activity."""
    import json
    import sqlite3
    knowledge = {}

    # First memory / active since
    try:
        db = ECHO_DIR / "echo_semantic_memory.sqlite"
        if db.exists():
            ...
            if row:
                knowledge["first_memory"] = row[0]
            count = conn.execute("SELECT COUNT(*) FROM memories").fetchone()[0]
            knowledge["memory_count"] = count
            conn.close()
    except Exception:
        pass

    # Standing task stats — total cycles, wins/losses
    try:
        tf = ECHO_DIR / "memory" / "standing_tasks.json"
        if tf.exists():
            data = json.loads(tf.read_text())
            knowledge["total_cycles"] = data.get("total_cycles", 0)
            tasks = data.get("tasks", [])
            knowledge["task_count"] = len([t for t in tasks if not t.get("disabled")])
            total_wins = sum(t.get("wins", 0) for t in tasks)
            total_losses = sum(t.get("losses", 0) for t in tasks)
            knowledge["task_wins"] = total_wins
            knowledge["task_losses"] = total_losses
    except Exception:
        pass

    # Trade P&L from cascade ledger
    try:
        lf = ECHO_DIR / "memory" / "cascade_ledger.json"
        if lf.exists():
            ledger = json.loads(lf.read_text())
            total_pl = sum(
                s.get("realized_pl", 0) for s in ledger.values() if isinstance(s, dict)
            )
            knowledge["realized_pl"] = round(total_pl, 2)
    except Exception:
        pass

    # Leads found
    try:
        leads_f = ECHO_DIR / "memory" / "demand_leads.json"
        if leads_f.exists():
            leads = json.loads(leads_f.read_text())
            knowledge["leads_found"] = len(leads)
            knowledge["leads_high_score"] = sum(1 for l in leads if l.get("score", 0) >= 7)
    except Exception:
        pass

    # Builds deployed
    try:
        reg_f = ECHO_DIR / "builds" / "registry.json"
        if reg_f.exists():
            reg = json.loads(reg_f.read_text())
            knowledge["builds_deployed"] = sum(1 for b in reg.values() if b.get("status") == "deployed")
            knowledge["builds_pending"] = sum(1 for b in reg.values() if b.get("status") == "pending")
    except Exception:
        pass
...
def build_self_awareness_block() -> str:
    ...
    # Self-knowledge — real history, not hallucination
    try:
        sk = get_echo_self_knowledge()
        sk_parts = []
        if sk.get("first_memory"):
            sk_parts.append(f"active since {sk['first_memory'][:10]}")
        if sk.get("memory_count"):
            sk_parts.append(f"{sk['memory_count']:,} memories stored")
        if sk.get("total_cycles"):
            sk_parts.append(f"{sk['total_cycles']:,} reasoning cycles")
        if sk.get("task_wins") is not None:
            sk_parts.append(f"{sk['task_wins']}W/{sk['task_losses']}L on tasks")
        if sk.get("realized_pl") is not None:
            sk_parts.append(f"${sk['realized_pl']:+,.2f} realized P&L")
        if sk.get("leads_found"):
            sk_parts.append(f"{sk['leads_found']} leads found ({sk.get('leads_high_score',0)} high-score)")
        if sk.get("builds_deployed"):
            sk_parts.append(f"{sk['builds_deployed']} scripts self-built")
        if sk_parts:
            lines.append("Self-knowledge: " + " | ".join(sk_parts))
    except Exception as e:
        lines.append(f"Self-knowledge failed: {e}")
