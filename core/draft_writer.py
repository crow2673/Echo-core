...
def create_draft(title, outline=""):
    ...
    try:
        import json as _j
        td = _j.loads(BASE / "memory/standing_tasks.json".read_text())
        tasks_str = "\n".join(
    ...
    try:
        ...
        cs = _json.loads(BASE / "memory/content_strategy.json".read_text())
        ...
        if next_item:
            ...
            BASE / "memory/content_strategy.json".write_text(_json.dumps(cs, indent=2))
    except Exception:
        pass
    try:
        wc = BASE / "memory/world_context.md".read_text()[:400]
    except Exception:
        wc = "world context unavailable"

    try:
        pub_count = len(list(BASE / "content/published".glob("*.md")))
    except Exception:
        pub_count = 9
