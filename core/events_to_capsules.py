...
def tick(max_lines: int = 50) -> int:
    ...
    for ln in new_lines:
        ...
        if ev.get("type") == "file_change":
            cap_id = f"EVENT:file_change:{base_ms}:{seq}"
            seq += 1
            capsules.append({
                "capsule_id": cap_id,
                "type": "event",
