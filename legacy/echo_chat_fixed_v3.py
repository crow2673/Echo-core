...
print("Also accepts: task, task please, reflection, reflect on progress")
print("File read: read filename.txt")

tm = TaskManager()

def quiet_load_all():
    # task_manager_v2 prints "Loaded XX events"; suppress it here
    buf = io.StringIO()
...
def normalize_command(text: str) -> str:
    t = text.strip().lower()
    # map common phrases -> canonical command words
    if t in ("task", "task please", "tasks please", "show task", "show my tasks", "task list"):
        return "tasks"
    if t in ("reflect", "reflection", "reflecting", "reflect on progress", "reflect please"):
        return "reflect"
    return text
